...
def antycheat(user_input: str, last_user_input: str, initial_len_of_input: int, guesses) -> typing.Tuple[str, int]:
    """
        ...
        :param guesses: сформана стрічка з підверджених букв, що вгадав компютер
    """
...
# початок додаткової логіки
    """"
        Лише за умови  guesses, що  літерами які підтвердив користувач у функції play_game, має хоча би 1 символ.
        Тоді перевіряється введені символи з останнім символом guesses, які не дорівнюють _ чи попередньому введенню
        
        Доки гравець не введе новий символ, який буде збігатись з підтвердженим, комп'ютер проситиме повторити спробу
    """
    if len(guesses) > 0:
        different_character: typing.List[bool] = [corrected_input[i] != guesses[-1]
                                                  for i in range(initial_len_of_input)
                                                  if corrected_input[i] != '_' and not last_user_input[i] == corrected_input[i]]

        has_different_character: bool = all(different_character) or reduce(lambda x, y: x != y, different_character)

        while has_different_character:
            print("Что-то тут не так.")
            print(f"Ты ввел другую букву. Я угадывал букву - {guesses[-1]}")
...
#Инициализируем все перед началом игры
    ...
    while is_playing:
        if was_correct:
            ...
            current_word, len_of_word = antycheat(current_word, last_word, len_of_word, guesses)
        # Добавив ще один параметр guesses у виклик функції antycheat
    ...
    print(f"Это было не сложно, мне понадобилось всего {len(guesses)} попыток! {guesses}")
